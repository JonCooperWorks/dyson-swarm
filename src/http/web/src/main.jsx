 ...
 *   2. Build a WardenClient with getToken bound to sessionStorage so
...
async function start() {
  ...
  const client = new WardenClient({ getToken: auth.getToken });
...
start().catch(err => {
  ...
  console.error('warden boot failed:', err);
});
