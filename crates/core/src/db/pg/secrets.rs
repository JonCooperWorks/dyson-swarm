...
#[derive(Debug, Clone)]
pub struct PgUserSecretStore {
    pool: PgPool,
}

impl PgUserSecretStore {
...
#[derive(Debug, Clone)]
pub struct PgSystemSecretStore {
    pool: PgPool,
}

impl PgSystemSecretStore {
    pub fn new(pool: PgPool) -> Self {
        Self { pool }
    }
}
...
#[async_trait]
impl UserSecretStore for PgUserSecretStore {
    ...
    async fn get(&self, user_id: &str, name: &str) -> Result<Option<String>, StoreError> {
        let row =
            sqlx::query("SELECT ciphertext FROM user_secrets WHERE user_id = $1 AND name = $2")
                .bind(user_id)
                .bind(name)
    ...
    async fn delete(&self, user_id: &str, name: &str) -> Result<(), StoreError> {
        sqlx::query("DELETE FROM user_secrets WHERE user_id = $1 AND name = $2")
            .bind(user_id)
            .bind(name)
            .execute(&self.pool)
            .await
    ...
    async fn list(&self, user_id: &str) -> Result<Vec<(String, String)>, StoreError> {
        let rows = sqlx::query(
            "SELECT name, ciphertext FROM user_secrets WHERE user_id = $1 ORDER BY name",
        )
        .bind(user_id)
        .fetch_all(&self.pool)
        .await
...
#[async_trait]
impl SystemSecretStore for PgSystemSecretStore {
    ...
    async fn get(&self, name: &str) -> Result<Option<String>, StoreError> {
        let row = sqlx::query("SELECT ciphertext FROM system_secrets WHERE name = $1")
            .bind(name)
            .fetch_optional(&self.pool)
            .await
            .map_err(map_sqlx)?;
    ...
    async fn delete(&self, name: &str) -> Result<(), StoreError> {
        sqlx::query("DELETE FROM system_secrets WHERE name = $1")
            .bind(name)
            .execute(&self.pool)
            .await
            .map_err(map_sqlx)?;
